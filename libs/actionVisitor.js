...
ActionVisitor.prototype = {
	 ...
	 */
	getComprcessStream:function(pipeOnStream){
		
		var rv = null;
		
		switch(this.getComprcessType()){
			case "deflate" :
	...
	send:function(content,statusCode,contentType,opts){
		...
		if(opts.gzip == true && (comprcessType = me.getComprcessType()) != ""){
			...
			me.setHeader("Content-Encoding", comprcessType);
			compressStream = me.getComprcessStream();
	...
	sendFile:function(fpath,statusCode,_opts){
		...
		if(opts.gzip == true && (comprcessType = me.getComprcessType()) != ""){
			...
			if(opts.isChunk){
				// trunk下发
				targetWStream = me.getComprcessStream(me.response);
			}else{
				// 不以trunk下发
				targetWStream = me.getComprcessStream();
				var contentLength=0
				var bufferArr = [];

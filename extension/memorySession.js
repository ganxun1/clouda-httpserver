...
var __gc = function(maxTimeout){
	...
	log.dev("Clouda-Session : start GC");
...
var sessionFactory = function(req,res){
	 ...
	 * 机器码, 长效追逐一台机器.
	 */
	var clientId = cookie.get("clouda-client-id");
	
	/**
	 * session, 一次访问
	 */
	var sessionid = cookie.get("clouda-session-id");
	
	if(typeof(clientId) != 'string' || clientId.length != 16){
		clientId = createClientId();
		var expires = new Date(Date.now() + clientExpires);
		cookie.set("clouda-client-id",clientId,{
	...
	if(typeof(sessionid) != 'string' || sessionid.length != 32){
		...
		cookie.set("clouda-session-id", sessionid, {
			path : "/"
		});
	}

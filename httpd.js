 ...
 */
var actions = {}, extensions = {} , filters = {};
...
httpd = _extend(new EventEmitter(),{
		...
		start : function(_conf){
			...
			if(_conf){
				...
				/**
				 * 这将合并后的conf对像,重新进行定义clouda-httpserver.
				 * 并重新利用config的watch行为启动server
				 */
				conf.autoStart = true;
				clouda.config.define("clouda-httpserver",conf);
				return;
			}
			...
			conf.mapping.push(conf.defaultAction || function(){
				this.sendError(new Error("httpserver : no action configure, unable to work!!"),500);
			});
...
clouda.plugin.define("clouda-httpserver",['clouda-log'],function(log,cb){
	
    if(server){
        return;
    }
	
    appName = clouda.resource.appName || "Clouda+";
    
	log.info("Initialize Clouda Http Server...");
	
    cb && cb(null,httpd);
    
    clouda.config.watch('clouda-httpserver',function(_conf){
    	...
    	if(conf){
    		log.info("overwrite the configure of clouda-httpserver before the server start;");
    	}
